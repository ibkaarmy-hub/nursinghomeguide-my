...
def load_sheet():
    """Load facility list from local CSV snapshot (generated by Google Drive MCP)."""
    if not LOCAL_CSV.exists():
        sys.exit(f"❌ {LOCAL_CSV} not found. Run the MCP export step first.")
    rows = list(csv.DictReader(LOCAL_CSV.open(encoding="utf-8")))
    print(f"  Loaded {len(rows)} facilities from {LOCAL_CSV}")
    return rows
...
def _apify_post(actor, payload):
    """Direct REST call to Apify run-sync endpoint. Returns dataset items list."""
    url = APIFY_RUN_SYNC.format(actor=actor, token=APIFY_TOKEN)
...
def apify_crawl_website(url, slug):
    """Fetch website content via apify~rag-web-browser. Returns list of page dicts."""
...
def apify_scrape_maps(facility, slug):
