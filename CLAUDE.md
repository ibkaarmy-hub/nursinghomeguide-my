...
├── _research/                             ← external research: market demand, user questions, competitors, pricing benchmarks (READ before writing new content)
├── facility/                              ← per-facility static pages (auto-generated by generate_facility_pages.py)
├── sitemap.xml                            ← auto-generated by generate_sitemap.py
├── robots.txt                             ← search-engine crawl directives
├── CNAME                                  ← GitHub Pages custom domain (nursinghomeguide.my)
└── stages/
    ├── 01-data/      — DONE (406 rows in sheet; ~350 live after status filtering)
    ├── 02-enrich/    — EDITORIALS DONE 100% (350/350 live facilities); Maps placeId 350/471; pricing + Details tab still pending
...
Before writing any new guide, area page, or content piece, read `_research/` to ground decisions in evidence (search demand, real user questions, competitor coverage gaps, verified pricing benchmarks). This is what differentiates our content from operator-marketing listicles.

Files:
- `_research/README.md` — index
- `_research/market-demand.md` — DOSM demographics, supply growth, policy gap
- `_research/user-questions.md` — what Malaysian families actually ask (Lowyat-sourced; Reddit thin for this topic)
- `_research/competitors.md` — landscape map + the gap we own (neutral, family-side, fact-based)
- `_research/pricing-benchmarks.md` — verified Klang Valley fee ranges, refresh annually

When new research comes in (new competitor pages, fresh forum threads, updated stats), update the relevant file and bump its `Last updated:` date.

## SEO infrastructure (updated 2026-05-10)

- Canonical host: `https://nursinghomeguide.my` (custom domain via Namecheap → GitHub Pages, CNAME committed)
- **786 per-facility static pages** across `/nursing-homes/`, `/assisted-living/`, `/home-care/`, `/day-care/` with baked-in OG / Twitter / canonical / LocalBusiness JSON-LD + `display:none` data block for AI crawlers (GPTBot, ClaudeBot, PerplexityBot). Generated by `generate_facility_pages.py`.
- Category routing: nursing home takes precedence over all other care types. JS classifier (`facility.html`) and Python generator (`generate_facility_pages.py`) are aligned — both check `!nursing home` before routing to day-care/AL/home-care.
- `generate_facility_pages.py` STATE_DIRS includes all 15 states — never wipes state listing pages on regeneration.
- Legacy `facility.html?slug=X` URLs redirect to the new clean URL form via `/facility/<slug>/`.
- **350/471 licensed facilities** have Google Maps `query_place_id` URLs (col T). Batch script: `batch_maps_placeid.py`. Resume-able via `batch_maps_results.json`.
- **Do NOT commit `existing_facilities.csv`** — generator always fetches from the live sheet. Local cache causes stale data bugs.
- `sitemap.xml` (~383 URLs) generated by `generate_sitemap.py` and submitted to Google Search Console.
- Weekly remote agent regenerates static pages + sitemap on Mondays at 09:00 KL time. Manage at https://claude.ai/code/routines
- Don't bump `style.css?v=` cache-buster unless CSS actually changed.
